...
const Authorization = () => {
  ...
  const [
    updateRole,
    { data: _roleData, isLoading: updateLoading, isSuccess: _updateSuccess },
            ...
            <div className="row g-3">
              <div className="col-sm-6">
                <label htmlFor="firstName" className="form-label">
                  Authorization (ROLE_{user?.data.user.role})
                </label>
                <select
                  defaultValue={user?.data.user.role}
                  className="form-select"
                  onChange={(event) =>
                    onUpdateRole({ role: event.target.value })
                  }
                  disabled={user?.data.user.role === "USER" || updateLoading}
                >
                  <option value={"USER"} key={"ROLE_USER"}>
                    ROLE_USER
                  </option>
                  <option value={"ADMIN"} key={"ROLE_ADMIN"}>
                    ROLE_ADMIN
                  </option>
                  <option value={"SUPER_ADMIN"} key={"ROLE_SUPER_ADMIN"}>
                    ROLE_SUPER_ADMIN
                  </option>
                  <option value={"MANAGER"} key={"ROLE_MANAGER"}>
                    ROLE_MANAGER
                  </option>
                  <option value={"PROFESSOR"} key={"ROLE_PROFESSOR"}>
                    ROLE_PROFESSOR
                  </option>
                </select>
